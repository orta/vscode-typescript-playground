...
export const createDesignSystem = (ts?: typeof import("typescript")) => {
  return (container: Element) => {
    ...
    const addEditorHoverToElement = (element: HTMLElement, pos: { start: number; end: number }, config: { type: "error" | "info" }) => {
    ...
    const button = (settings: { label: string; onclick?: (ev: MouseEvent) => void }) => {
      ...
      const join = document.createElement("vscode-button") as any
      // join.type = "button"
      // join.value = settings.label
      join.appearance = "secondary"
      join.innerText = settings.label
      if (settings.onclick) {
    ...
    const listDiags = (diags: DiagnosticRelatedInformation[]) => {
      ...
      diags.forEach((diag) => {
        ...
        if (typeof diag === "string") {
          ...
          if (!ts) {
            throw new Error("Can't handle this diag withtout typescript in the runtime")
          }
          li.textContent = ts.flattenDiagnosticMessageText(diag.messageText, "\n", 4)
        }
